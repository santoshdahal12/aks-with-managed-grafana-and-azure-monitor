# The template needs to be deployed in the same resource group as the Azure Managed Grafana workspace. as per 
#https://learn.microsoft.com/en-us/azure/azure-monitor/containers/prometheus-metrics-enable?tabs=terraform#enable-prometheus-metric-collection

# template is from https://github.com/Azure/prometheus-collector/blob/main/AddonTerraformTemplate/main.tf
...
resource "azurerm_monitor_workspace" "amw" {
  name                = var.azurerm_monitor_workspace
  resource_group_name = var.resource_group_name
...
# Managed Grafana
## ---------------------------------------------------
resource "azurerm_dashboard_grafana" "adg" {
  name                              = var.azurerm_dashboard_grafana
  resource_group_name               = var.resource_group_name
  location                          = data.azurerm_resource_group.rg.location
...
resource "azurerm_role_assignment" "grafana" {
  ...
  principal_id         = azurerm_dashboard_grafana.adg.identity[0].principal_id
}
 
# Add role assignment to Grafana so an admin user can log in
resource "azurerm_role_assignment" "grafana-admin" {
  scope                = azurerm_dashboard_grafana.adg.id
...
output "grafana_url" {
  value = azurerm_dashboard_grafana.adg.endpoint
}


resource "azurerm_monitor_data_collection_endpoint" "dce" {
  name                = "MSProm-${data.azurerm_resource_group.rg.location}-${var.cluster_name}"
  resource_group_name = var.resource_group_name
  location = data.azurerm_resource_group.rg.location
  kind                = "Linux"
}
...
resource "azurerm_monitor_data_collection_rule" "dcr" {
  name                        = "MSProm-${var.resource_group_name}-${var.cluster_name}"
  resource_group_name = var.resource_group_name
  location = data.azurerm_resource_group.rg.location
...
resource "azurerm_monitor_data_collection_rule_association" "dcra" {
  name                    = "MSProm-${data.azurerm_resource_group.rg.location}-${var.cluster_name}"
  target_resource_id      = data.azurerm_kubernetes_cluster.k8s.id
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_recording_rules_rule_group" {
  ...
  interval            = "PT1M"
  scopes              = [azurerm_monitor_workspace.amw.id,data.azurerm_kubernetes_cluster.k8s.id]
